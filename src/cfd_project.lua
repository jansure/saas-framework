...
-- TEST
-- 先转发到杀毒服务器"http://114.116.12.13:8866/scan/stream?name=test"
-- 读取请求体body
ngx.req.read_body()
local inheaders = ngx.req.get_headers()
local bodydata = ngx.req.get_body_data()
local httpc = http:new()
local res, err = httpc:request_uri("http://114.116.12.13:8866",
        {
            path = "/scan/stream?name=test",
            method = "POST",
            headers = inheaders,
            body = bodydata
        })
ngx.log(ngx.ERR, "----杀毒服务器res.body: ", res.body)
